...
$result = mysqli_query($koneksi,"SELECT * FROM `transaksi` join user on transaksi.idUser = user.idUser where status = 'Transaksi Selesai'");
                ...
                <center>
                    ...
                    <table>
                        ...
                        if (isset($_POST['submit'])) {
                            ...
                            $ambil = mysqli_query($koneksi,"SELECT * FROM `transaksi` join user on transaksi.idUser = user.idUser where status = 'Transaksi Selesai' and transaksi.kdTransaksi like '%".$kdTransaksi."%' ORDER BY `transaksi`.`tglTransaksi` DESC");
                            if ($ambil)
                            {
                        ...
                        $pagi = true;
                        $ambil = mysqli_query($koneksi,"SELECT * FROM `transaksi` join user on transaksi.idUser = user.idUser where status = 'Transaksi Selesai' ORDER BY `transaksi`.`tglTransaksi` DESC LIMIT $mulai, $halaman");
                        if ($ambil)
                        {
                            while($pecah = mysqli_fetch_assoc($ambil)){ 
                                $kdTransaksi = $pecah['kdTransaksi'];
