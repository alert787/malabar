...
$result = mysqli_query($koneksi,"SELECT * FROM `transaksi` join user on transaksi.idUser = user.idUser where status = 'Menunggu Pelunasan'");
            ...
            <div class="page-breadcrumb">
                <div class="row">
                    <div class="col-12 d-flex no-block align-items-center">
                        <h4 class="page-title">Menunggu pelunasan</h4>
                    </div>
                </div>
            </div>
            <!-- ============================================================== -->
                ...
                <center>
                    ...
                    <table>
                        ...
                        if (isset($_POST['submit'])) {
                            ...
                            $ambil = mysqli_query($koneksi,"SELECT * FROM `transaksi` join user on transaksi.idUser = user.idUser where status = 'Menunggu Pelunasan' and transaksi.kdTransaksi like '%".$kdTransaksi."%' ORDER BY `transaksi`.`tglTransaksi` DESC");
                            if ($ambil)
                            {
                        ...
                        $pagi = true;
                        $ambil = mysqli_query($koneksi,"SELECT * FROM `transaksi` join user on transaksi.idUser = user.idUser where status = 'Menunggu Pelunasan' ORDER BY `transaksi`.`tglTransaksi` DESC LIMIT $mulai, $halaman");
                        if ($ambil)
                        {
                            while($pecah = mysqli_fetch_assoc($ambil)){ 
                                $kdTransaksi = $pecah['kdTransaksi'];
