...
$result = mysqli_query($koneksi,"select * from sewa join transaksi on transaksi.kdTransaksi = sewa.kdTransaksi join detailsewa on detailsewa.kdSewa = sewa.kdSewa");
...
<head>
    ...
    <title>Admin | Penyewaan Barang</title>
    <!-- Custom CSS -->
...
<body>
    ...
    <div id="main-wrapper">
        ...
        <!-- Page wrapper  -->
        <!-- ============================================================== -->
        <div class="page-wrapper" style="background-color: #2a2b30; color: rgba(236, 240, 241,1.0); height: 100%;">
            <!-- ============================================================== -->
            ...
            <div class="page-breadcrumb">
                <div class="row">
                    <div class="col-12 d-flex no-block align-items-center">
                        <h4 class="page-title">Detail Penyewaan Barang</h4>
                    </div>
                </div>
            </div>
            <!-- ============================================================== -->
                ...
                <center>
                    ...
                    <table>
                        ...
                        if (isset($_POST['submit'])) {
                            ...
                            $ambil = mysqli_query($koneksi,"select * from sewa join transaksi on transaksi.kdTransaksi = sewa.kdTransaksi join detailsewa on detailsewa.kdSewa = sewa.kdSewa where transaksi.kdTransaksi like '%".$kdTransaksi."%' ORDER BY `transaksi`.`tglTransaksi` DESC");
                            if ($ambil)
                            {
                                while($pecah = mysqli_fetch_assoc($ambil)){ 
                                    $idBrg = $pecah['idBrg'];
                                    $brg = mysqli_query($koneksi,"select * from barang where idBrg = '$idBrg'");
                                    $barang = mysqli_fetch_assoc($brg);
                                    $kdTransaksi = $pecah['kdTransaksi'];
                                    $trf = mysqli_query($koneksi,"SELECT namaUser,tglTransaksi FROM `transaksi` join user on transaksi.idUser = user.idUser where kdTransaksi = '$kdTransaksi'");
                                    ...
                                    <tr>
                                        ...
                                        <td class="tab"><?php echo date("d-M-Y", strtotime($pecah['tglSewa'])),"-<br>",date("d-M-Y", strtotime($pecah['tglKembali'])) ?></td>
                        ...
                        {
                            $pagi = true;
                            $ambil = mysqli_query($koneksi,"select * from sewa join transaksi on transaksi.kdTransaksi = sewa.kdTransaksi join detailsewa on detailsewa.kdSewa = sewa.kdSewa ORDER BY `transaksi`.`tglTransaksi` DESC LIMIT $mulai, $halaman");
                            if ($ambil)
                            {
                                while($pecah = mysqli_fetch_assoc($ambil)){ 
                                    $idBrg = $pecah['idBrg'];
