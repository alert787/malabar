...
$result = mysqli_query($koneksi,"SELECT * FROM `transaksi` join user on transaksi.idUser = user.idUser where status IN ('Terkonfirmasi','Terkonfirmasi Pengembalian','Menunggu Pengambilan Barang')");
...
<head>
    ...
    <title>Admin | Menunggu Selesai</title>
    <!-- Custom CSS -->
    ...
    <style type="text/css">
        ...
        table {border-collapse: collapse; width: 90%; text-align: center; font-size: 14px; color: white;}
            ...
            <div class="page-breadcrumb">
                <div class="row">
                    <div class="col-12 d-flex no-block align-items-center">
                        <h4 class="page-title">Transaksi Terkonfirmasi</h4>
                    </div>
                </div>
            </div>
            <!-- ============================================================== -->
                ...
                <center>
                    ...
                    <div style="width: 90%; text-align: left;">
                        <div class="inblock" style="width: 49.5%">
                            <form method="post">
                                <input type="text" name="cari" class="inblock" placeholder="Cari Kode Transaksi">
                    ...
                    <table>
                        ...
                        if (isset($_POST['submit'])) {
                            ...
                            $ambil = mysqli_query($koneksi,"SELECT * FROM `transaksi` join user on transaksi.idUser = user.idUser where status IN ('Terkonfirmasi','Terkonfirmasi Pengembalian','Menunggu Pengambilan Barang') and transaksi.kdTransaksi like '%".$kdTransaksi."%' ORDER BY `transaksi`.`tglTransaksi` DESC");
                            if ($ambil)
                            {
                                while($pecah = mysqli_fetch_assoc($ambil)){ 
                                    $kdTransaksi = $pecah['kdTransaksi'];

                                    $sewa = mysqli_query($koneksi,"SELECT statusBayar FROM `sewa` where kdTransaksi = '$kdTransaksi'");
                                    $sw = mysqli_fetch_assoc($sewa);

                                    ?>
                                    <tr>
                                        <td><?php echo $no ?></td>                      
                                        <td><?php echo $kdTransaksi ?></td>
                                        <td><?php echo $pecah['namaUser'] ?></td>
                                        <td><?php echo date("d-M-Y", strtotime($pecah['tglTransaksi'])) ?></td>
                                        <td><?php echo $pecah['status']; ?></td>
                                        <td><a href="detailtranup.php?id=<?php echo $kdTransaksi ?>" class="tombol">Detail</a></td>                       
                                        <td><?php echo "Rp. ",number_format($pecah['totalHarga']) ?></td>
                                    </tr>


                                    <?php
                                    $total = $total+$pecah['totalHarga'];
                                    $no++;
                                }
                            }
                            ?> 
                            ...
                            $pagi = true;
                            $ambil = mysqli_query($koneksi,"SELECT * FROM `transaksi` join user on transaksi.idUser = user.idUser where status IN ('Terkonfirmasi','Terkonfirmasi Pengembalian','Menunggu Pengambilan Barang') ORDER BY `transaksi`.`tglTransaksi` DESC LIMIT $mulai, $halaman");
                            if ($ambil)
                            {
                                while($pecah = mysqli_fetch_assoc($ambil)){ 
                                    $kdTransaksi = $pecah['kdTransaksi'];
