...
<head>
    ...
    <style type="text/css">
        ...
        .tombol {padding-top: 3px; padding-bottom: 3px; padding-right: 5px; padding-left: 5px; font-size: 10; background-color: rgba(231, 76, 60,1.0); color: white; border-radius: 3px; border: 0px;}
        ...
        table {border-collapse: collapse; width: 90%; text-align: center; color: white; font-size: 12px; vertical-align: middle;}
...
<body>
    ...
    $res = "SELECT tglLaporan FROM `laporan` ORDER BY `laporan`.`tglLaporan` DESC LIMIT 1";
    ...
    <div id="main-wrapper">
        ...
        <div class="page-wrapper" style="background-color: #2a2b30; color: rgba(236, 240, 241,1.0)">
            ...
            <div class="container-fluid">
                <br>
                <center>
                    <p>Terakhir diperbaharui : <?php echo date("d-m-Y", strtotime($assc['tglLaporan'])) ?></p><br>
                    <div style="width: 90%; text-align: left;">
                        <div class="inblock" style="width: 49.5%">
                            <form method="post">
                                <input type="date" name="cari" class="inblock">
                                s/d 
                                <input type="date" name="cari2" class="inblock">
                                <button class="cari" name="submit">Cari</button>                            
                            </form>
                        </div>
                        <div class="inblock" style="width: 50%; text-align: right;">
                            <button onclick="myfunction2()" class="tomboltam">CETAK LAPORAN</button>
                        </div>
                    </div>
                    ...
                    <table style="margin-top: 25px">
                        ...
                        if (isset($_POST['submit'])) {
                            ...
                            if ((is_null($nama)) or (is_null($nama2))) {
                                echo '<script>
                                swal({
                                    title: "Oops!",
                            ...
                            if ($nama > $nama2) {
                                $brg = mysqli_query($koneksi,"SELECT * FROM laporan where tglLaporan between '$nama2' and '$nama' ORDER BY `tglLaporan` ASC");
                            }
                            else{
                                $brg = mysqli_query($koneksi,"SELECT * FROM laporan where tglLaporan between '$nama' and '$nama2' ORDER BY `tglLaporan` ASC");    
                            }
                            
                            while($barang = mysqli_fetch_assoc($brg)){
                                ?>
                                <tr>
                                    <td><?php echo $no ?></td>                      
                                    <td><?php echo date_format(date_create($barang['tglLaporan']),"d-m-Y") ?></td>
                                    <td><a href="../../file/<?php echo $barang['fileLaporan'] ?>"><?php echo $barang['fileLaporan'] ?></a></td>
                                    <td><a href="detailtranup.php?id=<?php echo $kdTransaksi ?>" class="tombol">Hapus</a></td>
                                </tr>
                                <?php
                                $no++;
                            }
                            ...
                            $pagi = true;
                            $brg = mysqli_query($koneksi,"SELECT * FROM laporan ORDER BY `tglLaporan` ASC LIMIT $mulai, $halaman");
                            while($barang = mysqli_fetch_assoc($brg)){
                                ?>
                                <tr>
                                    <td><?php echo $no ?></td>                      
                                    <td><?php echo date_format(date_create($barang['tglLaporan']),"d-m-Y") ?></td>
                                    <td><a href="../../file/<?php echo $barang['fileLaporan'] ?>"><?php echo $barang['fileLaporan']; ?></a></td>
                                    <td><button onclick="myfunction('<?php echo $barang["fileLaporan"]; ?>')" class="tombol">Hapus</button></td>
                                </tr>
                                <?php
                                $no++;
                            }
    ...
    <script type="text/javascript">
        function myfunction(ini)
        {
            ...
            .then((willDelete) => {
              if (willDelete) {
                window.location = "hpsl.php?id="+id; 
            }
        });
        }
    </script>
