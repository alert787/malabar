		...
		<div class="page-wrapper" style="background-color: #2a2b30; color: rgba(236, 240, 241,1.0)">
			...
			<div class="container-fluid">
					...
					<form method="post" enctype="multipart/form-data">
						<table>
							<tr>
								...
								<td>:</td>
								<td>
									<input type="text" name="namabrg" placeholder="Nama Barang...">
								</td>
							</tr>
							...
							<tr>
								...
								<td>
									<select type="text" name="kategori" placeholder="Nama Barang...">
										...
										<option value="Alat Makan & Minum">Alat Makan & Minum</option>
										...
										<option value="Hammock & Sleeping Bag">Hammock & Sleeping Bag</option>
						...
						<button class="tombol" name="tambah">Tambah Barang</button></form>
					</center>
					<br>
				</div>
				<?php
				if (isset($_POST['tambah'])) {
					...
					$query = "SELECT max(idBrg) as maxKode FROM barang";
					$hasil = mysqli_query($koneksi,$query);
					$data = mysqli_fetch_array($hasil);
					$kodetransaksi = $data['maxKode'];
					$noUrut = (int) substr($kodetransaksi, 2, 9);
					$noUrut++;
					$char = 'BR';

					$kodetrf = $char.str_pad($noUrut, 7, "0", STR_PAD_LEFT);
					if (is_null($data)) {
						$kodetrf = 'BR'.'0000001';
					}
					...
					if((is_null($nama)) or (is_null($kategori)) or (is_null($hargabeli)) or (is_null($hargajual)) or (is_null($hargasewa)) or (is_null($jumlah)) or (is_null($deskripsi)) or (is_null($gambar)))
					{
						echo '<script>
						swal({
							title: "Oops!",
					...
					{
						$tgl = date('Y-m-d');
						$res = mysqli_query($koneksi,"insert into barang values ('$kodetrf','$nama','$kategori','$hargajual','$hargasewa','$deskripsi','$gambar','$jumlah')");
						...
						$res = mysqli_query($koneksi,"insert into detailbeli values ('','$jumlah','$hargabeli','$total','$kodetransaksi','$kodetrf')");
						if ($hargajual!=0) {
							echo '<script>
							swal({
								title: "Berhasil!",
