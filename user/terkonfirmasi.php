  ...
  <center>
    ...
    <div style="width: 90%; align-self: center;" class="inblock">
      ...
      <div style="width: 80%; display : inline-block; border-left: 1px solid gray;">
        ...
        $ambil = mysqli_query($koneksi,"select * from transaksi where idUser='$idUser' and status IN ('Terkonfirmasi','Transaksi Selesai','Terkonfirmasi Pengembalian','Menunggu Pengambilan Barang') ORDER BY `transaksi`.`tglTransaksi` DESC");   
        $no = 0;
        if ($ambil)
        {
          $count = mysqli_query($koneksi,"select count(kdTransaksi) from transaksi where idUser='$idUser' and status IN ('Terkonfirmasi','Transaksi Selesai')");
